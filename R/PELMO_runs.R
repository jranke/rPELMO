...
#'
#' As a side effect, an R data file (period_rPELMO.rda) is generated in each
...
#' @importFrom pfm FOCUS_GW_scenarios_2012
...
#' time_1 <- system.time(
#'   PECgw_1 <- PELMO_runs(runs_1, psm_dir = system.file("testdata", package = "rPELMO"),
...
#'                     win = c("Cha", "Ham", "Kre")))
#' PECgw_2 <- PELMO_runs(runs_2, psm_dir = system.file("testdata", package = "rPELMO"),
                       ...
                       evaluate = TRUE, overwrite = FALSE)
{
  ...
  if (evaluate) {
    rPELMO_PECgw <- evaluate_PELMO(runs, version = version, PELMO_base = PELMO_base)
    return(rPELMO_PECgw)
  } else {
    invisible(NULL)
  }
...
evaluate_PELMO <- function(runs, version = "5.5.3", PELMO_base = "auto")
{
  ...
  for (run in runs) {
    ...
    for (crop in crops) {
      ...
      rPELMO_PECgw[[psm]][[crop]] <- matrix(nrow = length(scenarios), ncol = length(acronyms),
      ...
      for (scenario in scenarios) {
        ...
        for (acronym in acronyms) {
          ...
          results_rPELMO[[acronym]]$periods <- periods
          PECgw <- focus_80th(periods$conc)

          results_rPELMO[[acronym]]$focus <- PECgw

          rPELMO_PECgw[[psm]][[crop]][scenario, acronym] <- round(PECgw, 3)
        }
        save(results_rPELMO, file = file.path(run_dir, "period_rPELMO.rda"))
      }
    }
  }
  return(rPELMO_PECgw)
